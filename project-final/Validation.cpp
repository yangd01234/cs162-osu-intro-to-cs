...
** Author: Derek Yang
** Date:12/2/2017
** Description:This is the validation class.  This class will validate
user input.
*********************************************************************/
...
void Validation::getValidInt(std::string ques, int &inputVar, int max, int min){
    ...
    //check size and if it is an int NOTE: will not reach max/min
...
void Validation::getValidStr(std::string ques, std::string &inputStr) {
    std::cout<<ques;
    std::cin>>inputStr;

    while(std::cin.fail()){
        std::cout<<"Invalid, enter a string"<<std::endl;
        std::cin.clear();
